...
const Contact = () => {
  ...
  const handleSend = async (e) => {
    ...
    if (username === "") {
      ...
      // All validation passed, send to Formspree
      try {
        ...
        const res = await fetch("https://formspree.io/f/xwpbaadk", {
          method: "POST",
        ...
        if (res.ok) {
          ...
          setSubject("");
          setMessage("");
          setTimeout(() => setSuccessMsg(""), 5000); // Success message disappears after 5 seconds
        } else {
          setErrMsg("Failed to send message. Please try again later.");
          setTimeout(() => setErrMsg(""), 5000); // Optionally clear error after 5 seconds
        }
      } catch (error) {
        setErrMsg("Failed to send message. Please try again later.");
      }
